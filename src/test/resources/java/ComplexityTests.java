...
// Courtesy of https://www.sonarsource.com/docs/CognitiveComplexity.pdf

public class CognitiveComplexityExamples {

    void logicalSequenceExample1() {
        if (a               // +1 for 'if'
                && b && c   // +1
                || d || e   // +1
                && f        // +1
    ...
    void myMethod2() {
        Runnable r = () -> {        // +0 (nesting becomes 1)
    ...
    private void addVersion(final Entry entry, final Transaction txn)
        ...
        while (true) {                                                  // +1
            try {
                ...
                try {
                    final long depends = _persistit.getTransactionIndex().wwDependency(re.getVersionHandle(), txn.getTransactionStatus(), SharedResource.DEFAULT_MAX_WAIT_TIME);
